...
from red_model import  RedNet2DV3L_3x3_Residual_2B,\
        ...
        RedNet2DV3L_3x3_Residual_3B_Att_C,\
        RedNet2DV3L_3x3_Residual_5B,\
        RedNet2DV3L_3x3_BottleNetResidual_3B,\
        RedNet2DV3L_3x3_Residual_2B_Att,\
        RedNet2DV3L_3x3_Residual_2B_LS_Att,\
        RedNet2DV3L_3x3_Residual_3B_MS_Att
#        
...
parser.add_argument('--model', choices=['rednet2B','rednet2BLSAtt', 'rednet3BMSAtt','rednet3B','rednetBottle3B', 'rednet5B','rednet2BC','rednet3BC','rednet3BAtt','rednet2BAtt','rednet3BAtt_C'],default='rednet3B')
parser.add_argument('--input', required=True)
parser.add_argument('--output', required=True)
parser.add_argument('--weights', default=os.path.join(os.path.split(sys.argv[0])[0],"Sargassum21b_128_DMCI.pth"))
...
parser.add_argument('--no_nor', action='store_true', help='copie NB')
parser.add_argument('--save_bytes', action='store_true', help='save float and bytes')
...
#  
output_channels = 1
nb_classe=output_channels
print("Create modele" ,args.model)
...
if args.model == 'rednet2B' :
    ...
    model=RedNet2DV3L_3x3_Residual_5B(channels,nb_classe,dims=dim,init=False)
elif args.model == 'rednetBottle3B' :
    model=RedNet2DV3L_3x3_BottleNetResidual_3B(channels,nb_classe,dims=dim,init=True) 
...
elif  args.model == 'rednet2BLSAtt' :
    model= RedNet2DV3L_3x3_Residual_2B_LS_Att(channels,nb_classe,dims=dim )    
elif  args.model == 'rednet3BAtt_C' :
    model= RedNet2DV3L_3x3_Residual_3B_Att_C(channels,nb_classe,dims=dim )
    
elif  args.model == 'rednet3BMSAtt' :
    model= RedNet2DV3L_3x3_Residual_3B_MS_Att(channels,nb_classe,dims=dim, train=False )    
...
def load_image(  path=None,channels=21,normalize=False):
        ...
        print("CC Image size b" ,bands,"Xsize",rows,"Ysize",cols )
        print("CC Image size b" ,channels,"Xsize",rows,"Ysize",cols )
        print("CC path = " ,path)
        if bands <  channels :
            print("ERROR   BAND  NUMBER not enough bands comparing to requested channels ",channels,bands)
            quit()
...
resimage = np.zeros( ( height,width ),dtype=np.float32)
...
#                output=torch.squeeze(output,1)
        ...
        # no_nan=pred[pred!=0.01]
        # print("cont",len(no_nan))
        # print(pred)
        # print("pred ",np.count_nonzero(pred))
        # print(no_nan)
        # print("np.where(pred!=-0.01).size", np.where(pred!=-0.01)[0].size)
        nb_=np.where(pred>0)[0].size
        nbSup +=nb_
        # print("predit > 0", nb_)
...
print("total  > 0 ",nbSup)
print('\nsave to ', args.output  )   
if args.save_bytes == True :
    resimage[resimage<0]=0
    resimage*=255/0.25
    resimage[resimage>255]=255
    resimage=np.round(resimage)
    #resimage=np.clip(resimage,0,255).astype(np.byte)
    resimage=np.clip(resimage,0,255).astype(np.uint8)
    nom2=args.output[:-4]+"_Bytes.tif"
