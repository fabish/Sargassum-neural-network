...
echo "gdalwarp -geoloc -t_srs EPSG:4326 coord.vrt $output -srcnodata 255 -overwrite" 

time gdalwarp -multi -geoloc -t_srs EPSG:4326 coord.vrt /tmp/geo$$.tif -srcnodata 255 -overwrite
...
echo "Coordenadas: ${$lon} ${$lat} ${$lon2} ${$lat2}"
